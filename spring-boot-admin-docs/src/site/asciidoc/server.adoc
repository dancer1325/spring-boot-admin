...
* if Spring Boot Admin server is running behind a reverse proxy ->
  ** configure the public url -- `spring.boot.admin.ui.public-url` --
  ** & reverse proxy terminates the https connection -> configure -- `server.forward-headers-strategy=native`
    *** check https://docs.spring.io/spring-boot/docs/current/reference/htmlsingle/#howto-use-tomcat-behind-a-proxy-server[Spring Boot Reference Guide]
